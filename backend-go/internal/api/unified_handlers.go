...
func (d *UnifiedDeps) GetUpcomingUnified(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Get("debug") == "1" {
		baseCtx = context.WithValue(baseCtx, "unifiedDebug", true)
	}
	items, debugInfo, err := svc.GetUpcoming(baseCtx, objectID, parsed.Hours, parsed.Sources, parsed.Limit)
	if err != nil {
		http.Error(w, "Failed to get unified items: "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	resp := models.UnifiedUpcomingResponse{Items: items, Hours: parsed.Hours, Total: len(items), ServerTimestamp: time.Now().Unix(), Stats: stats, Debug: debugInfo}
...
func (d *UnifiedDeps) GetUnifiedCalendar(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Failed to get calendar: "+err.Error(), http.StatusInternalServerError)
		return
	}
	...
	if len(parsedCal.Sources) > 0 {
		sf := map[string]bool{}
		for _, s := range parsedCal.Sources {
			sf[s] = true
		}
	...
	if parsedCal.Limit > 0 {
		...
		for day := range cal.Days {
			...
			if remain < len(items) {
				cal.Days[day] = items[:remain]
			}
			collected = parsedCal.Limit
			if collected >= parsedCal.Limit {
				break
			}
		}
	}
