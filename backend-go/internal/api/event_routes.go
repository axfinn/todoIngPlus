...
func (d *EventDeps) CreateEvent(w http.ResponseWriter, r *http.Request) {
	userID := GetUserID(r)
	if userID == "" { writeJSONError(w, http.StatusUnauthorized, "unauthorized", "Unauthorized"); return }
	if ct := r.Header.Get("Content-Type"); ct != "" {
		lc := strings.ToLower(ct)
		if !strings.HasPrefix(lc, "application/json") { writeJSONError(w, http.StatusBadRequest, "content_type", "Unsupported Content-Type (expect application/json)"); return }
	}
	objectID, err := primitive.ObjectIDFromHex(userID)
	if err != nil { writeJSONError(w, http.StatusBadRequest, "user_id", "Invalid user ID (legacy account). Please re-register to use events/reminders."); return }
	...
	if trimmed == "" { writeJSONError(w, http.StatusBadRequest, "empty_body", "Request body is empty"); return }
	...
	if strictErr != nil {
		...
		if err2 := json.Unmarshal([]byte(trimmed), &m); err2 != nil {
			trunc := trimmed; if len(trunc) > 500 { trunc = trunc[:500] + "..." }
		...
		mapAssignStr := func(key string, dst *string) { if v, ok := m[key]; ok { if s, ok2 := v.(string); ok2 { *dst = s } } }
		mapAssignInt := func(key string, dst *int) { if v, ok := m[key]; ok { switch val := v.(type) { case float64: *dst = int(val); case int: *dst = val } } }
		mapAssignBool := func(key string, dst *bool) { if v, ok := m[key]; ok { if b, ok2 := v.(bool); ok2 { *dst = b } } }
		mapAssignStr("title", &raw.Title)
		mapAssignStr("description", &raw.Description)
		mapAssignStr("event_type", &raw.EventType); if raw.EventType == "" { mapAssignStr("eventType", &raw.EventType) }
		mapAssignStr("event_date", &raw.EventDate); if raw.EventDate == "" { mapAssignStr("eventDate", &raw.EventDate) }
		if raw.RawEventDate == "" { mapAssignStr("raw_event_date", &raw.RawEventDate) }
		mapAssignStr("recurrence_type", &raw.RecurrenceType); if raw.RecurrenceType == "" { mapAssignStr("recurrenceType", &raw.RecurrenceType) }
		if v, ok := m["recurrence_config"]; ok { if mm, ok2 := v.(map[string]interface{}); ok2 { raw.RecurrenceConfig = mm } }
		mapAssignInt("importance_level", &raw.ImportanceLevel); if raw.ImportanceLevel == 0 { mapAssignInt("importanceLevel", &raw.ImportanceLevel) }
		mapAssignStr("location", &raw.Location)
		mapAssignBool("is_all_day", &raw.IsAllDay); if !raw.IsAllDay { mapAssignBool("isAllDay", &raw.IsAllDay) }
		// tags 处理
		if v, ok := m["tags"]; ok {
			if arr, ok2 := v.([]interface{}); ok2 {
				for _, el := range arr { if s, ok3 := el.(string); ok3 { raw.Tags = append(raw.Tags, s) } }
			}
		}
		observability.LogWarn("CreateEvent strict decode failed, used fallback mapping error=%v", strictErr)
	}

	if raw.Title == "" { writeJSONError(w, http.StatusBadRequest, "field_title", "Title is required"); return }
	if raw.EventType == "" { writeJSONError(w, http.StatusBadRequest, "field_event_type", "Event type is required"); return }
	if raw.EventDate == "" && raw.RawEventDate != "" { raw.EventDate = raw.RawEventDate }
	if raw.EventDate == "" { writeJSONError(w, http.StatusBadRequest, "field_event_date", "Event date is required"); return }

	parsedDate, err := parseFlexibleEventDate(raw.EventDate, raw.IsAllDay)
	if err != nil {
		if len(raw.EventDate) == 16 && raw.EventDate[10] == 'T' { if t2, e2 := time.ParseInLocation("2006-01-02T15:04", raw.EventDate, time.Local); e2 == nil { parsedDate = t2.UTC(); err = nil } }
		if err != nil { writeJSONError(w, http.StatusBadRequest, "date_parse", "Invalid event_date format: "+err.Error()); return }
	}
	...
	if err != nil { writeJSONError(w, http.StatusInternalServerError, "create_failed", fmt.Sprintf("Failed to create event: %v", err)); return }
...
func parseFlexibleEventDate(s string, isAllDay bool) (time.Time, error) {
	...
	if len(s) == 10 { // 只有日期
		if isAllDay { s = s + "T00:00" } else { s = s + "T00:00" }
	}
	...
	if matchLen := len(s); matchLen == 16 { // 形如 2006-01-02T15:04
		if lt, err := time.ParseInLocation("2006-01-02T15:04", s, time.Local); err == nil { return lt.UTC(), nil }
	}
...
func (d *EventDeps) GetEventOptions(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
	    observability.LogWarn("GetEventOptions list error user=%s err=%v", userID, err)
			http.Error(w, fmt.Sprintf("Failed to list events: %v", err), http.StatusInternalServerError); return }
