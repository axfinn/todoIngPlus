...
func (r *mongoReminderRepo) Insert(ctx context.Context, m *models.Reminder) error {
	if m == nil { return errors.New("nil reminder") }
	if m.ID.IsZero() { m.ID = primitive.NewObjectID() }
	now := time.Now(); if m.CreatedAt.IsZero() { m.CreatedAt = now }; m.UpdatedAt = now
	// 计算 next_send
	var ev models.Event
	if err := r.events().FindOne(ctx, bson.M{"_id": m.EventID, "user_id": m.UserID}).Decode(&ev); err == nil {
		m.NextSend = m.CalculateNextSendTime(ev)
	}
	_, err := r.coll().InsertOne(ctx, m); return err
}
...
func (r *mongoReminderRepo) GetWithEvent(ctx context.Context, userID, reminderID primitive.ObjectID) (*models.ReminderWithEvent, error) {
	...
	if err = cur.All(ctx, &res); err != nil { return nil, err }
	if len(res)==0 { return nil, errors.New("reminder not found") }
	return &res[0], nil
}
...
func (r *mongoReminderRepo) UpdateFields(ctx context.Context, userID, reminderID primitive.ObjectID, set map[string]interface{}, recomputeNext bool) (*models.Reminder, error) {
	...
	if err != nil { return nil, err }
	if res.MatchedCount == 0 { return nil, errors.New("reminder not found") }
	var rm models.Reminder
	if err = r.coll().FindOne(ctx, bson.M{"_id": reminderID}).Decode(&rm); err != nil { return nil, err }
	if recomputeNext {
		var ev models.Event
...
func (r *mongoReminderRepo) Delete(ctx context.Context, userID, reminderID primitive.ObjectID) error {
	...
	if res.DeletedCount == 0 { return errors.New("reminder not found") }
	return nil
}
...
func (r *mongoReminderRepo) ListPagedWithEvent(ctx context.Context, userID primitive.ObjectID, page, pageSize int, activeOnly bool) (*models.ReminderListResponse, error) {
	...
	if err != nil { return nil, err }
	var cnt []bson.M; _ = curCount.All(ctx, &cnt); curCount.Close(ctx)
	var total int64
	if len(cnt) > 0 { switch v := cnt[0]["total"].(type){ case int32: total=int64(v); case int64: total=v } }
...
func (r *mongoReminderRepo) Upcoming(ctx context.Context, userID primitive.ObjectID, hours int) ([]models.UpcomingReminder, error) {
	if hours <= 0 { hours = 24 }
	now := time.Now(); end := now.Add(time.Duration(hours)*time.Hour)
	...
	if err != nil { return nil, err }
	defer cur.Close(ctx)
	parse := func(v interface{}) *time.Time { switch tv := v.(type){ case primitive.DateTime: t:=tv.Time(); return &t; case time.Time: return &tv; case *time.Time: return tv; case string: if tv=="" {return nil}; if t1,e:=time.Parse(time.RFC3339, tv); e==nil {return &t1}; if t2,e:=time.Parse("2006-01-02 15:04:05", tv); e==nil {return &t2}; if t3,e:=time.Parse("2006-01-02", tv); e==nil {return &t3}; }; return nil }
	var out []models.UpcomingReminder
	for cur.Next(ctx){ var raw bson.M; if cur.Decode(&raw)!=nil { continue }
		id, _ := raw["_id"].(primitive.ObjectID)
		title, _ := raw["event_title"].(string)
		evDate := parse(raw["event_date"]); nx := parse(raw["next_send"]); if nx==nil || nx.Year()<2000 { continue }
		msg := ""; if cm,ok := raw["custom_message"].(string); ok && cm!="" { msg = cm } else if title!="" { msg = "提醒: "+title } else { msg = "提醒" }
		imp := 0; switch v := raw["importance_level"].(type){ case int32: imp=int(v); case int64: imp=int(v); case int: imp=v }
		base := nx; if evDate!=nil && !evDate.IsZero() { base = evDate }
		daysLeft := int(base.Sub(now).Hours()/24)
		ed := time.Time{}; if evDate!=nil && evDate.Year()>=2000 { ed = *evDate }
...
func (r *mongoReminderRepo) Pending(ctx context.Context) ([]models.ReminderWithEvent, error) {
	...
	cur, err := r.coll().Aggregate(ctx, pipeline); if err != nil { return nil, err }
	defer cur.Close(ctx)
	var list []models.ReminderWithEvent; if err = cur.All(ctx, &list); err!=nil { return nil, err }
	return list, nil
}

func (r *mongoReminderRepo) ToggleActive(ctx context.Context, userID, reminderID primitive.ObjectID) (bool, error) {
	var rm models.Reminder
	if err := r.coll().FindOne(ctx, bson.M{"_id": reminderID, "user_id": userID}).Decode(&rm); err != nil { if err==mongo.ErrNoDocuments { return false, errors.New("reminder not found") }; return false, err }
	newVal := !rm.IsActive
...
func (r *mongoReminderRepo) Snooze(ctx context.Context, userID, reminderID primitive.ObjectID, minutes int) error {
	...
	if err != nil { return err }
	if res.MatchedCount == 0 { return errors.New("reminder not found") }
	return nil
}

func (r *mongoReminderRepo) MarkSent(ctx context.Context, reminderID primitive.ObjectID) error {
	var rm models.Reminder
	if err := r.coll().FindOne(ctx, bson.M{"_id": reminderID}).Decode(&rm); err != nil { return err }
	var ev models.Event
	if err := r.events().FindOne(ctx, bson.M{"_id": rm.EventID}).Decode(&ev); err != nil { return err }
	now := time.Now(); next := rm.CalculateNextSendTime(ev)
...
func (r *mongoReminderRepo) CreateImmediateTest(ctx context.Context, userID, eventID primitive.ObjectID, message string, delaySeconds int) (*models.Reminder, *models.Event, error) {
	if delaySeconds < 0 { delaySeconds = 0 }; if delaySeconds > 3600 { delaySeconds = 3600 }
	var ev models.Event
	if err := r.events().FindOne(ctx, bson.M{"_id": eventID, "user_id": userID}).Decode(&ev); err != nil { if err==mongo.ErrNoDocuments { return nil, nil, errors.New("event not found") }; return nil, nil, err }
	now := time.Now()
	...
	ns := now.Add(time.Duration(delaySeconds)*time.Second); rm.NextSend = &ns
	if _, err := r.coll().InsertOne(ctx, rm); err != nil { return nil, nil, err }
	return rm, &ev, nil
}
...
func BuildPreview(event *models.Event, eventFound bool, advanceDays int, times []string, next *time.Time) (string, []string) {
	var errs []string
	if !eventFound { errs = append(errs, "event not found") }
	var b strings.Builder
	if event != nil { b.WriteString(event.Title) } else { b.WriteString("(event)") }
	if event != nil { b.WriteString(" @ "); b.WriteString(event.EventDate.Format("2006-01-02 15:04")) }
	b.WriteString(fmt.Sprintf(" | advance %d day(s) | times %v", advanceDays, times))
	if next != nil { b.WriteString(" => next "); b.WriteString(next.Format("2006-01-02 15:04")) }
...
func (r *mongoReminderRepo) computePreview(ctx context.Context, userID, eventID primitive.ObjectID, advanceDays int, times []string) (*models.Event, *time.Time, error) {
	...
	if err != nil { if err==mongo.ErrNoDocuments { return nil, nil, nil }; return nil, nil, err }
...
// PreviewReminderOutput 提供给上层的复合结果
 type PreviewReminderOutput struct { Event *models.Event; Next *time.Time; Text string; ValidationErrs []string }
