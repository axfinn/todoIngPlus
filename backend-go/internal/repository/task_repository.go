...
func (r *mongoTaskRepo) Insert(ctx context.Context, t *models.Task) error {
	if t == nil { return errors.New("nil task") }
	now := time.Now(); t.CreatedAt = now; t.UpdatedAt = now
	res, err := r.coll().InsertOne(ctx, t)
	if err != nil { return err }
	if oid, ok := res.InsertedID.(primitive.ObjectID); ok { t.ID = oid.Hex() }
	return nil
}
...
func (r *mongoTaskRepo) List(ctx context.Context, userID, status string, page, limit int64) ([]models.Task, int64, error) {
	...
	for cur.Next(ctx) { var m models.Task; if cur.Decode(&m)==nil { list = append(list, m) } }
...
func (r *mongoTaskRepo) FindByID(ctx context.Context, userID, id string) (*models.Task, error) {
	...
	if oid, err := primitive.ObjectIDFromHex(id); err == nil { filter["$or"] = append(filter["$or"].([]bson.M), bson.M{"_id": oid}) }
	var m models.Task
	if err := r.coll().FindOne(ctx, filter).Decode(&m); err != nil { return nil, err }
	return &m, nil
}
...
func (r *mongoTaskRepo) UpdatePartial(ctx context.Context, userID, id string, set bson.M) (*models.Task, error) {
	...
	filter := bson.M{"createdBy": userID, "$or": []bson.M{{"_id": id}}}
	if oid, err := primitive.ObjectIDFromHex(id); err == nil { filter["$or"] = append(filter["$or"].([]bson.M), bson.M{"_id": oid}) }
	if _, err := r.coll().UpdateOne(ctx, filter, bson.M{"$set": set}); err != nil { return nil, err }
