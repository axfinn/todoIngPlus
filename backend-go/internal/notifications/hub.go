...
func NewHub() *Hub { return &Hub{subs: make(map[primitive.ObjectID]map[chan models.Notification]struct{})} }
...
func (h *Hub) Subscribe(ctx context.Context, userID primitive.ObjectID) chan models.Notification {
    ...
    h.mu.Lock()
    if _, ok := h.subs[userID]; !ok { h.subs[userID] = make(map[chan models.Notification]struct{}) }
...
func (h *Hub) Unsubscribe(userID primitive.ObjectID, ch chan models.Notification) {
    ...
    if m, ok := h.subs[userID]; ok {
        if _, ok2 := m[ch]; ok2 { delete(m, ch); close(ch) }
...
func (h *Hub) Broadcast(n models.Notification) {
    ...
    if m, ok := h.subs[n.UserID]; ok {
        for ch := range m { select { case ch <- n: default: } }
    }
}
