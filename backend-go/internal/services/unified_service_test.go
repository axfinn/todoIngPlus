...
func TestComputeUpcomingSignature(t *testing.T) {
    ...
    if _, err := svc.GetUpcoming(ctx, dummyUser, 24, []string{"event"}, 10); err == nil {
...
func TestBuildUpcomingItems(t *testing.T) {
    ...
    if len(items) != 5 { t.Fatalf("expected 5 items got %d", len(items)) }
    ...
    if len(itemsEv) != 1 || itemsEv[0].Source != "event" { t.Errorf("event filter failed: %+v", itemsEv) }
    // Case 3: limit truncation
    limited := buildUpcomingItems(now, end, nil, 3, events, reminders, priority, normals)
    if len(limited) != 3 { t.Errorf("limit not applied: %d", len(limited)) }
    // Ensure first 3 keep ordering from full list
    if limited[0].ID != pt1.ID || limited[1].ID != events[0].ID.Hex() || limited[2].ID != reminders[0].ID.Hex() { t.Errorf("limit ordering mismatch: %+v", limited) }
}
