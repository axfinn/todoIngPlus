...
func NewNotificationService(db *mongo.Database) *NotificationService { return &NotificationService{db: db} }
...
func (s *NotificationService) Create(ctx context.Context, in models.NotificationCreate) (models.Notification, error) {
    ...
    if _, err := s.collection().InsertOne(ctx, n); err != nil { return models.Notification{}, err }
    return n, nil
}
...
func (s *NotificationService) List(ctx context.Context, userID primitive.ObjectID, unreadOnly bool, limit int) ([]models.Notification, error) {
    ...
    if unreadOnly { filter["read_at"] = bson.M{"$exists": false} }
    ...
    for cur.Next(ctx) { var n models.Notification; if err := cur.Decode(&n); err == nil { out = append(out, n) } }
    return out, cur.Err()
}
...
func (s *NotificationService) MarkRead(ctx context.Context, userID, id primitive.ObjectID) error {
    ...
    if res.MatchedCount == 0 { return errors.New("not_found") }
    return nil
}
